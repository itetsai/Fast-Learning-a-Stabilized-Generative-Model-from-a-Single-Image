...
from FastSinGAN.config import get_arguments
...
if __name__ == '__main__':
    ...
    if opt.train_mode == "generation" or opt.train_mode == "retarget" or opt.train_mode == "animation":
        if opt.train_mode == "animation":
            opt.min_size = 20
        from FastSinGAN.training_generation import *
        ...
        if opt.fine_tune:
            ...
            if not os.path.exists(opt.naive_img):
                ...
                exit()
        from FastSinGAN.training_harmonization_editing import *
    ...
    copytree(osp.join(current_path, "FastSinGAN"), osp.join(dir2save, "FastSinGAN"))
    ...
    with open(osp.join(dir2save, 'time.txt'), 'w') as f:
        f.write(str(elapsed_time))
        f.close()
    
    from evaluation_func.draw_table import Loss2Picture
    import cv2
