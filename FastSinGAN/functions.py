...
from FastSinGAN.imresize import imresize, imresize_in, imresize_to_shape
...
def sample_random_noise(depth, reals_shapes, opt, netG):
    noise = []
        ...
        if d == 0:
            ...
            if opt.train_mode == "generation" or opt.train_mode == "animation":
                if netG.is_shrink==True:
                    noise.append(generate_noise([opt.nfc, reals_shapes[d][2] + opt.num_layer * 2 + netG.noise_add*2,
                                                reals_shapes[d][3] + opt.num_layer * 2 + netG.noise_add*2],
                                                device=opt.device).detach())
                else:
...
def calc_gradient_penalty(netD, real_data, fake_data, LAMBDA, device):
    ...
    if  MSGGan:
        ...
        disc_interpolates = interpolates_D
    else:
        alpha = torch.rand(1, 1)
...
def generate_dir2save(opt):
    ...
    dir2save = opt.model_save_path+'/{}/'.format(training_image_name)
